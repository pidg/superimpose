...
detect_size() {
    ...
    # A6: 298 x 420 points
        ...
        echo "A6"
    # A7: 210 x 298 points
    elif (( $(echo "$w > 200 && $w < 220 && $h > 288 && $h < 308" | bc -l) )) || \
         (( $(echo "$h > 200 && $h < 220 && $w > 288 && $w < 308" | bc -l) )); then
        echo "A7"
    else
        echo "UNKNOWN"
    fi
...
case "$PAGE_SIZE" in
    ...
    *)
        ...
        echo "Supported sizes: A4, A5, A6, A7"
        exit 1
        ;;
esac
...
# Rotate input pages if needed (A5 and A7)
...
for ((i=0; i<NUM_SHEETS; i++)); do
    ...
    # If rotation needed and it's 1x2 layout (A5), reverse the page order
    # For 2x4 layout (A7), pdfjam handles the rotated pages correctly without reversal
    if [ "$NEEDS_ROTATION" = true ] && [ "$NUP" = "1x2" ]; then
        # Reverse the arrays for A5 only
